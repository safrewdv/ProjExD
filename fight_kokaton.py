...
class Beam:   # 練習１
    def __init__(self,bird: Bird):
        self.img = pg.transform.rotozoom(pg.image.load("fig/beam.png"), 0, 2.0)  #　注意
        self.rct: pg.Rect = self.img.get_rect()
...
class  Explosion:
    def __init__(self,bomb: Bomb):
        ...
        self.rct: pg.Rect = self.img.get_rect() 
        self.rct.centery = bomb.rct.center #爆発した爆弾のrct.centerに座標を設定
        self.life = 5  #表示時間（爆発時間）lifeを設定
    def update(self):
    ...
    while True:
        for event in pg.event.get():
            if event.type == pg.QUIT:
                return
            if event.type == pg.KEYDOWN and event.key == pg.K_SPACE:  # 練習１
                beam = Beam(bird)
